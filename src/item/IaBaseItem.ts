...
> implements IaItemData<ItemMetaType, ItemFileMetaType> {
    ...
    public get simplelists(): { holdings: { [key: string]: IaSimplelistEntry; }; } | undefined {
     ...
     */
    public hash(): string {
        const itemData: Partial<IaItemData<ItemMetaType, ItemFileMetaType>> = { ...this.itemData };
        for (const key of EXCLUDED_ITEM_DATA_KEYS) {
            delete itemData[key];
        }
        const hash = createHash('md5');
        hash.update(stableStringify(itemData));
